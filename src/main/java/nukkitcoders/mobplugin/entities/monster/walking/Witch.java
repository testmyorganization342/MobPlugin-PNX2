...
public class Witch extends WalkingMonster {
    ...
    public void attackEntity(Entity player) {
        if (attackDelay > ATTACK_TICKS && distanceSquared(player) <= 8) { // they attack only beginning from 8 blocks away ...
            attackDelay = 0;
            if (player.isAlive() && !player.closed) {
                ...
                if (distance(player) <= 8 && !player.hasEffect(Effect.SLOWNESS)) {
                    thrownPotion.potionId = Potion.SLOWNESS;
                } else if (player.getHealth() >= 8) {
                    thrownPotion.potionId = Potion.POISON;
                } else if (distance(player) <= 3 && !player.hasEffect(Effect.WEAKNESS) && Utils.rand(0, 4) == 0) {
                ...
                if (launch.isCancelled()) {
                    ...
                    level.addSound(this, Sound.MOB_WITCH_THROW);
                }
            }
        } else {
            attackDelay++;
